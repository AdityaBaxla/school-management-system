<template>
      ...
      <table class="min-w-full border-collapse">
        <thead>
          <tr class="bg-gray-200">
            <th class="px-4 py-2 text-left">Foli #</th>
  ...
  const students    = ref([{id: 1, name:"dummy", age: 10, class: "X"}, {id: 2, name: "bunny", age: 20, class:"V"}]);
  const loading     = ref(false);
  const error       = ref(null);
  const creating    = ref(false);
  const newStudent  = ref({ name: '', age: null, class: '' });
  ...
  try {
    ...
    // reset form
    newStudent.value = { name: '', rollNo: null, class: '' };
